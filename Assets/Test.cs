...
public class Test : MonoBehaviour
{
    ...
    private StatusCallback serverStatusCallback;
    private StatusCallback clientStatusCallback;
    NetworkingUtils serverNetworkingUtils = new NetworkingUtils();
        ...
        StatusCallback statusCallback = (ref StatusInfo info) =>
        {
            switch (info.connectionInfo.state)
            {
                ...
                case ConnectionState.Connecting:
                    ...
                    break;
                case ConnectionState.Connected:
                    Debug.Log("Server has been connected - ID: " + info.connection + " " + info.connectionInfo.address.GetIP());
                    break;
                case ConnectionState.ClosedByPeer:
                //No break here I assume to log the disconnection error
                case ConnectionState.ProblemDetectedLocally:
                    server.CloseConnection(info.connection);
                    Debug.Log("Server has been disconnected - ID: " + info.connection + " " + info.connectionInfo.address.GetIP());
                    break;
            }
        };

        //Sets server moderating callback
        serverNetworkingUtils.SetStatusCallback(statusCallback);
    ...
    private void RunClientSetUp()
    {
        ...
        //Defined status callback for server monotering
        StatusCallback clientStatusCallback = (ref StatusInfo info) =>
        {
            switch (info.connectionInfo.state)
            {
                case ConnectionState.None:
                    break;
                case ConnectionState.Connected:
                    Debug.Log("Client connected to server - ID: " + clientConnection);
                    break;
                case ConnectionState.ClosedByPeer:
                //No break here I assume to log the disconnection error
                case ConnectionState.ProblemDetectedLocally:
        ...
        //Sets client moderating callback
        clientNetworkingUtils.SetStatusCallback(clientStatusCallback);

        Address address = new Address();

        //Sets client address on port 5000
